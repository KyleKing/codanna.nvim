...
local function make_entry(item)
    ...
    if norm.kind then display = string.format("[%s] %s", norm.kind, norm.name) end
    if norm.file then
...
local function create_live_picker(title, search_fn, opts)
    ...
    local function do_search(query)
        ...
        search_fn(query, opts, function(data, err)
            if pending_query and pending_query ~= query then return end
    ...
    local function on_input_change()
        ...
        if not prompt_bufnr or not vim.api.nvim_buf_is_valid(prompt_bufnr) then return end
        ...
        debounce_timer:start(M.config.debounce_ms, 0, vim.schedule_wrap(function() do_search(query) end))
    end
...
function M.find_callers(opts)
    ...
    codanna.find_callers_async(symbol, {}, function(data, err)
        ...
        if #results == 0 then codanna.notify_empty_results(symbol) end
