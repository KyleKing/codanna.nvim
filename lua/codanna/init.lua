...
local function get_picker()
  if M.config.preferred_picker then
    ...
    if mod then
      ...
      if ok and next(picker) then
        return picker, M.config.preferred_picker
      end
    end
  end

  local ok, snacks_picker = pcall(require, "codanna.snacks")
  if ok and next(snacks_picker) then
    return snacks_picker, "snacks"
  end

  local ok2, telescope_picker = pcall(require, "codanna.telescope")
  if ok2 and next(telescope_picker) then
    return telescope_picker, "telescope"
  end

  local ok3, mini_picker = pcall(require, "codanna.mini")
  if ok3 and next(mini_picker) then
    return mini_picker, "mini"
  end
...
function M.find_implementations(opts)
  ...
  picker.find_implementations(opts)
end
...
function M.setup(opts)
  ...
  if ok and snacks_mod.setup then
    snacks_mod.setup()
  end
end
